...
import { Palette, Bot, ArrowLeft, Eye, Check, Loader2, Smartphone, Monitor } from 'lucide-react';
...
const ThreeDChatPage: React.FC = () => {
  ...
  const [theme, setTheme] = useState<ThemeConfig>({
    ...
    profileSubtitle: 'Delivery executive',
    profileBorderColor: '#f97316',
  ...
  const [agentConfig, setAgentConfig] = useState<AgentConfig>({
    ...
    systemInstruction: 'You are Mark Hendry, a high-end delivery executive assistant. Be polite, precise, and helpful. Use a modern professional tone.',
    knowledgeBase: '',
    temperature: 0.7,
    crawledUrls: []
  });
  ...
  useEffect(() => {
    const loadExistingAgent = async () => {
      ...
      try {
        ...
        console.error("Failed to load 3D agent unit:", err);
      } finally {
        setIsInitialLoading(false);
      }
    };
    ...
    return (
      <div className="h-screen w-screen bg-[#fcfcfd] flex flex-col items-center justify-center gap-4">
        ...
        <div className="flex items-center gap-2 text-blue-400 font-bold text-xs uppercase tracking-[0.3em]">
          <Loader2 className="w-4 h-4 animate-spin" /> Rendering Geometry...
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-1 bg-slate-100 border border-slate-200 rounded-2xl p-1">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${
              activeTab === 'design' ? 'bg-white text-blue-600 shadow-sm border border-slate-200' : 'text-slate-500 hover:text-slate-900'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-6 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${
              activeTab === 'agent' ? 'bg-white text-blue-600 shadow-sm border border-slate-200' : 'text-slate-500 hover:text-slate-900'
            }`}
          >
      ...
      <main className="flex-1 flex overflow-hidden relative">
        <div className="flex w-full h-full">
            ...
            <div className="flex-1 bg-slate-100 relative overflow-hidden flex items-center justify-center p-8">
                ...
                <div className="relative z-10 w-full max-w-[450px] flex flex-col items-center gap-6">
                   <div className="flex items-center gap-4 mb-4 bg-white/50 backdrop-blur-md px-4 py-2 rounded-full border border-white/50 shadow-sm">
                      <button className="p-2 text-blue-600 bg-white rounded-full shadow-sm"><Smartphone className="w-4 h-4" /></button>
                      <button className="p-2 text-slate-400 hover:text-slate-600 transition-colors"><Monitor className="w-4 h-4" /></button>
                   </div>
                   <div className="w-[340px] h-[680px] transition-all duration-700">
